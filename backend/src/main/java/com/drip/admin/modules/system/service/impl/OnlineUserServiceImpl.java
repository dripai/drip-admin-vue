...
public class OnlineUserServiceImpl implements OnlineUserService {
    ...
    @Override public PageResult<OnlineUserVo> page(OnlineUserQuery query) { Map<String, String> params = new HashMap<>(); params.put("page", String.valueOf(query.pageOrDefault())); params.put("pageSize", String.valueOf(query.pageSizeOrDefault())); if (query.getUsername() != null) params.put("username", query.getUsername()); if (query.getIp() != null) params.put("ip", query.getIp()); var page = onlineSessionService.page(params); return new PageResult<>(page.list().stream().map(OnlineUserServiceImpl::toVo).toList(), page.total(), page.page(), page.pageSize()); }
    @Override public OnlineUserVo detail(String tokenId) { return toVo(onlineSessionService.detail(tokenId)); }
    @Override public void kickout(String tokenId) { if (java.util.Objects.equals(tokenId, StpUtil.getTokenValue())) throw new BusinessException(400000, "???????????????"); StpUtil.logoutByTokenValue(tokenId); onlineSessionService.remove(tokenId); }
    private static OnlineUserVo toVo(Map<String, Object> row) { return new OnlineUserVo(string(row.get("tokenId")), longValue(row.get("userId")), string(row.get("username")), string(row.get("realName")), string(row.get("deviceType")), string(row.get("ip")), string(row.get("userAgent")), string(row.get("loginAt")), string(row.get("lastActiveAt"))); }
