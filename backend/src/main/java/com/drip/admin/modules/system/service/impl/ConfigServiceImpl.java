...
public class ConfigServiceImpl extends ServiceImpl<SysConfigMapper, SysConfigEntity> implements ConfigService {
    ...
    private static String typeOrDefault(String valueType) { return valueType == null || valueType.isBlank() ? "string" : valueType; }
    private static void validateValue(String valueType, String value) {
        if ("boolean".equals(valueType) && !("true".equalsIgnoreCase(value) || "false".equalsIgnoreCase(value))) throw new BusinessException(400000, "configValue must be true or false");
        if ("number".equals(valueType)) {
            try {
                Double.parseDouble(value);
            } catch (NumberFormatException ex) {
                throw new BusinessException(400000, "configValue must be number");
            }
        }
    }
}
