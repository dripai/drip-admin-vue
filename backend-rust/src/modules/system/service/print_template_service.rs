...
#[derive(Debug,Deserialize,Serialize,utoipa::ToSchema)]#[serde(rename_all(serialize="camelCase",deserialize="snake_case"))]pub struct PrintTemplate{pub id:I64String,pub code:String,pub name:String,pub paper_type:String,pub template_json:String,pub status:i32,pub deleted:i32}
...
#[cfg(test)]
mod tests {
    use super::PrintTemplate;

    #[test]
    fn accepts_database_column_names() {
        let template: PrintTemplate = serde_json::from_str(
            r#"{"id":"1","code":"demo","name":"Demo","paper_type":"A4","template_json":"{}","status":1,"deleted":0}"#,
        )
        .unwrap();
        assert_eq!(template.paper_type, "A4");
        assert_eq!(serde_json::to_value(template).unwrap()["paperType"], "A4");
    }
}
